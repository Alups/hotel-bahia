...
DataTable.ext.buttons.print = {
	...
	action: function ( e, dt, button, config ) {
		...
		html += '<tr style="font-size: 1.5rem !important; font-weight: 100 !important;"><th colspan="'+data.header.length+'">Bahia Hotel<br>Bldg. 664 Waterfront Rd cor McKinley St, Subic Bay Freeport Zone, Olongapo, Philippines, 2222<br>(047) 252 2894'+config.repeatingHead.title+'</th></tr>';
		...
		// Get the current page number from DataTable's pagination
		var currentPage = dt.page.info().page;

		// Calculate the starting and ending index for the current page
		var startIndex = currentPage * rowsPerPage;
		var endIndex = Math.min(startIndex + rowsPerPage, data.body.length);
		...
		var sum = count - 1;
		html += '<tfoot><td colspan="12">Total number of bookings: '+sum+'</td></tfoot>';
		...
		var head = '<title>Placed Orders Report</title>';
