...
if(isset($_POST['submit'])){
   ...
   if($select_admin->rowCount() > 0){
      ...
      if($pass != $cpass){
         ...
         $insert_admin = $conn->prepare("INSERT INTO `users`(name, email, password, confirmation) VALUES(?,?,?,?)");
         $insert_admin->execute([$name, $email, $cpass, $confimation]);
...
<section class="accounts">
   ...
   <div id="myModal1" class="modal">
      ...
      <div class="modal-content">
         ...
         <section class="add-products">
            <h1 class="heading">add new user account</h1>
            
               <form action="" method="post" class="needs-validation" novalidate>
                  <div class="flex">
                  <div class="inputBox">
                     <span>Name <i class="notice">*</i></span>
                     <input type="text" name="name" required placeholder="Enter your name" maxlength="20"  class="box form-control" oninput="this.value = this.value.replace(/\s/g, '')"> 
                  </div>
                  <div class="inputBox">
                     <span>Email <i class="notice">*</i></span>
                     <input type="text" name="email" required placeholder="Enter your email" maxlength="20"  class="box form-control" oninput="this.value = this.value.replace(/\s/g, '')"> 
                  </div>
                  <div class="inputBox">
                     <span>Password <i class="notice">*</i></span>
                     <input type="password" name="pass" required placeholder="Enter your password" maxlength="20"  class="box form-control" oninput="this.value = this.value.replace(/\s/g, '')"> 
                  </div>
                  <div class="inputBox">
                     <span>Confirm Password <i class="notice">*</i></span>
                     <input type="password" name="cpass" required placeholder="Confirm your password" maxlength="20"  class="box form-control" oninput="this.value = this.value.replace(/\s/g, '')"> 
                  </div>
                  </div>
   ...
   <div class="box-container">
         <?php	
         $select_user = $conn->prepare("SELECT * FROM `users` WHERE deactive = 0");
         $select_user->execute();
      ...
      <thead class="thd">
         <tr>
         <th scope="col" class='table-header' width="30%">First Name</th>
         <th scope="col" class='table-header' width="30%">Last Name</th>
         <th scope="col" class='table-header' width="30%">Email</th>
         <th scope="col" class='table-header' width="20%">Number</th>
         <th scope="col" class='table-header' width="1%">Action</th>
         </tr>
      </thead>
      <tbody id='table-body'>
         <?php
         ...
         <tr>
            ...
            <button type="button" class="delete-btn" data-bs-toggle="modal" data-bs-target="#deleteModal" data-bs-whatever="<?php  echo $row['id'];?>&name=<?php echo $row['name']?>"><i class="fas fa-trash-alt"></i></button>
            </td>
...
<script>
   ...
   document.getElementById("link").setAttribute("href","users_accounts.php?delete=" + idkoto);
   })
